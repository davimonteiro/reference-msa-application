...
public class ShoppingCart {
    ...
    public ShoppingCart incorporate(CartEvent cartEvent) {
        ...
        Flux<CartEvent.CartEventType> validCartEventTypes =
                Flux.fromStream(Stream.of(CartEvent.CartEventType.ADD_ITEM,
                        CartEvent.CartEventType.REMOVE_ITEM));
            ...
            productMap.put(cartEvent.getProductId(),
                    productMap.getOrDefault(cartEvent.getProductId(), 0) +
                            (cartEvent.getQuantity() * (cartEvent.getCartEventType()
                                    .equals(CartEvent.CartEventType.ADD_ITEM) ? 1 : -1)));
        }
     ...
     *
     * @param eventType is the {@link CartEvent.CartEventType} to evaluate
     * @return a flag indicating whether or not the event is terminal
     */
    public static Boolean isTerminal(CartEvent.CartEventType eventType) {
        return (eventType == CartEvent.CartEventType.CLEAR_CART || eventType == CartEvent.CartEventType.CHECKOUT);
    }
